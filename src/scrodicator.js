 ...
 */
LGL.UI.Scrodicator = Class.create({
    initialize: function() {
        var args = $A(arguments); argL = args.length;
        var area = $(args[0]) || $$('.scrodicatable').first();
        if (!area) return;
        area.scrodicator = this;
        var defOptions = {
            capProvider: {
        ...
        if (options.capProvider == 'form-label') options.capProvider = new LGL.UI.Scrodicator.FormLabelCaptionProvider(area);
        ...
        var beacon = new Element('b', {className: 'scrodi-beacon'});
        area.appendChild(beacon);

        var up = new Element('b', {className: 'scrodi-up'});
        var down = new Element('b', {className: 'scrodi-down'});
        up.hide(); down.hide();
        area.parentNode.appendChild(up);
        area.parentNode.appendChild(down);
        up.style.zIndex = down.style.zIndex = area.style.zIndex + LGL.UI.ZIndexDelta.Scrodicator;
...
LGL.UI.Scrodicator.FormLabelCaptionProvider = Class.create({
...
var js = /scrodicator\.js(\?.*)?$/;
$$('script[src]').findAll(function(s) {
  return s.src.match(js);
}).each(function(s) {
  autoload = s.src.match(/\?.*autoload.*/);
  if (autoload) {
    $$('.scrodicatable').each(function(el){LGL.UI.Scrodicator(el);});
  }
  scrollbar = s.src.match(/\?.*autoload=scrollbar*/);
  if (scrollbar) {
      $$('.scrodicatable').each(function(el){LGL.UI.Scrollbar(el);});
  }
});
